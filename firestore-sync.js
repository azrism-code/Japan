...
(() => {
  ...
  async function bootstrap(user){const tripRef=db.collection('trips').doc(TRIP),snap=await tripRef.get();if(!snap.exists){if((C.ownerEmail||'').toLowerCase()!==String(user.email||'').toLowerCase())throw new Error('Only the configured owner can create this trip');shareToken=ensureShareToken();await tripRef.set({ownerUid:user.uid,ownerEmail:user.email,editorUids:[],editorEmails:(C.editorEmails||[]).map(x=>x.toLowerCase()),shareToken,createdAt:new Date().toISOString()});setRole('owner')}else{const d=snap.data();shareToken=d.shareToken||'';setRole(d.ownerUid===user.uid?'owner':((d.editorUids||[]).includes(user.uid)||(d.editorEmails||[]).includes(String(user.email||'').toLowerCase()))?'editor':'viewer')}
