const CACHE='japan-trip-v9-12d';
const CORE=['./','./index.html','./manifest.json','./icon.svg','./styles.css','./app.js','./index.htm','./enhancements-v9.11.js','./walking-v9.12.js','./enhancements-v9.12.js','./state-fix-v9.12d.js'];
...
self.addEventListener('fetch',event=>{
  ...
  if(url.origin===self.location.origin && /\/app\.js$/.test(url.pathname)){
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response=>{
      ...
      const base=await response.text(); let extra=''; let walking=''; let shopping=''; let statefix='';
      ...
      try{const r=await fetch('./state-fix-v9.12d.js',{cache:'no-store'});if(r.ok)statefix=await r.text();}catch(e){}
      return new Response(base+'\n\n'+extra+'\n\n'+walking+'\n\n'+shopping+'\n\n'+statefix,{status:200,headers:{'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store'}});
    }).catch(()=>caches.match(event.request)));
    return;
  }
  if(url.origin===self.location.origin && /\/(index\.html|styles\.css|index\.htm|enhancements-v9\.11\.js|walking-v9\.12\.js|enhancements-v9\.12\.js|state-fix-v9\.12d\.js)$/.test(url.pathname)){
