// Japan Trip v9.8 loader
(async function bootJapanTrip(){
  try{
    const res=await fetch('./index.htm?v=98',{cache:'no-store'});
    ...
    installExpenses();
    document.title='Japan Trip 2026 · v9.8';
    const ver=document.querySelector('header .logo small'); if(ver) ver.textContent='2026 · v9.8';
    document.documentElement.dataset.appReady='v9.8';
  }catch(err){
  ...
  if('serviceWorker' in navigator){
    try{
      const reg=await navigator.serviceWorker.register('./service-worker.js?v=98');
      reg.update();
    }catch(e){console.warn('SW registration failed',e);}
  }
})();

// ---------- Expenses & Budget v9.8 ----------
const EXP_KEY='japanTrip_expenses_v1';
const EXP_SETTINGS_KEY='japanTrip_expense_settings_v1';

const EXP_DEFAULTS=[
  {name:'טיסה',status:'שולם',payment:'אשראי',currency:'USD',amount:3166,category:'✈️ טיסות',included:true,note:''},
...
function expLoad(){
  try{const x=JSON.parse(localStorage.getItem(EXP_KEY));if(Array.isArray(x))return x;}catch(e){}
  return EXP_DEFAULTS.map((x,i)=>({...x,id:Date.now()+i}));
}
...
function expRender(){
  ...
  root.innerHTML=`
    ...
    <div class="exp-rates"><h3>שערים ותקציב</h3><div class="exp-rate-grid">
...
function expCard(x,s){
  const native=x.currency==='ILS'?`${new Intl.NumberFormat('he-IL').format(x.amount)} ₪`:`${x.currency==='JPY'?'¥':x.currency==='USD'?'$':'€'}${new Intl.NumberFormat('he-IL').format(x.amount)}`;
  return `<div class="exp-card ${x.included===false?'alt':''}"><div class="exp-card-head"><div class="grow"><h4>${expEsc(x.category)} · ${expEsc(x.name)}</h4><div class="exp-meta">${expEsc(x.status)} · ${expEsc(x.payment||'אשראי')} · ${native}</div>${x.note?`<div class="exp-note">${expEsc(x.note)}</div>`:''}</div><div class="exp-amount">${expMoney(expIls(x,s))}</div></div><div class="exp-actions"><button onclick="expCycleStatus(${x.id})">שנה סטטוס</button><label><input type="checkbox" ${x.included!==false?'checked':''} onchange="expToggle(${x.id},this.checked)"> כלול בתקציב</label><button class="del" onclick="expDelete(${x.id})">מחק</button></div></div>`;
}
...
function expDelete(id){if(!confirm('למחוק את ההוצאה?'))return;expSave(expLoad().filter(x=>x.id!==id),expSettings());expRender();}
function expToggle(id,v){const a=expLoad(),x=a.find(x=>x.id===id);if(x)x.included=v;expSave(a,expSettings());expRender();}
function expCycleStatus(id){const a=expLoad(),x=a.find(x=>x.id===id),st=['הערכה','מוזמן','שולם'];if(x)x.status=st[(st.indexOf(x.status)+1)%st.length];expSave(a,expSettings());expRender();}
